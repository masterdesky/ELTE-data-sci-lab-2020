...
def load_HPX(file, field=1):
    ...
    hpx, _ = hp.read_map(file, field=field,
                         dtype=np.float64, h=True, memmap=True)
...
def plot_cmb(proj, cmap=None, c_min=None, c_max=None, cbar=False,
    ...
    with mpl.rc_context({
        ...
        fig, ax = plt.subplots(figsize=(8, 4), dpi=200, facecolor='black')
        ax.axis('off')
        ...
        #ax.set_xlabel(r'Angle $[^\circ]$',  fontsize=12, fontweight='bold')
        #ax.set_ylabel(r'Angle $[^\circ]$', fontsize=12, fontweight='bold')
        ...
        if cbar:
            ...
            cbar = plt.colorbar(mappable=im, orientation='horizontal', cax=cax)
            cbar.ax.tick_params(labelsize=12, size=0)
            cbar.set_label(r'Temperature [$\mu$K]', fontsize=12)
        if save:
            os.makedirs(out, exist_ok=True)
            fig.savefig(Path(out, save_filename), format='pdf', transparent=True,
...
def plot_spectrum(ell, Dl, DlTT,
    ...
    axes.plot(ell, DlTT, label=r'$\Lambda$-CDM simulation',
              color=cm.magma(0.65), lw=3, ls='--')

    axes.set_xlabel(r'$\ell$', fontsize=axislabelsize, fontweight='bold', color='white')
    axes.set_ylabel(r'$D_{\ell}$ [$\,\mu$K$^2\,$]', fontsize=axislabelsize, fontweight='bold', color='white')
